build {
  ...
  source "amazon-ebs.windows-server-2019" {
    name     = "msibuild-windows-server-2019-2.6"
    ami_name = "msibuild-windows-server-2019-2.6-1"
  }
  ...
  provisioner "powershell" {
    ...
    inline = ["C:/Windows/Temp/scripts/build-deps.ps1 -workdir C:\\buildbot\\windows-server-2019-static-msbuild -openvpn_ref release/2.5 -openvpn_build_ref release/2.5 -openvpn_gui master -openssl openssl -debug"]
  }
  provisioner "powershell" {
